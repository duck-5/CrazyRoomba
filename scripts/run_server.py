...
def main():
    ...
    controller.default_mode = args.mode

    if args.mock:
        asyncio.run(controller.connect(mock=True))
    elif args.auto_connect:
        try:
            asyncio.run(controller.connect(port=args.serial_port, mock=False))
    ...
    print(f"  * Mock Simulator    : {'ENABLED' if args.mock else 'DISABLED'}")
    print("=" * 65)
